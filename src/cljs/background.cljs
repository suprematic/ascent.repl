...
(defn assert-string [string]
	(if-not (string? string)
		(throw js/Error "string parameter expected")))

(defn set-port [destination tab-id port]
	{:pre [(assert-string tab-id), (assert-string destination)]}
...
(defn get-port [destination tab-id]
	{:pre [(assert-string tab-id), (assert-string destination)]}
	(get-from-map (get-from-map connections destination) tab-id))
...
(.addListener js/chrome.tabs.onUpdated
	(fn [tab-id change-info tab]
		(if ("complete" = (:status change-info))
			(set-tab-info tab-id {:agent-info nil :url (:url tab)})
			(if (get-port "repl" tab-id)
				(if (@debug) (log/debug (format "to repl:%s" tab-id (get-tab-info tab-id))))
				(.postMessage (get-port "repl" tab-id) {:type "tab-info" :info (get-tab-info tab-id)})))))
